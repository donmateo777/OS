...
def producmanual(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('inventario') # Redirige a inventario para ver el nuevo producto
        else:
            messages.error(request, 'Error al agregar el producto. Por favor, revisa los datos.')
    else:
        form = ProductoForm()
